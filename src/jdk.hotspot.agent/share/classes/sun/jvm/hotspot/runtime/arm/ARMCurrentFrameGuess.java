...
import sun.jvm.hotspot.debugger.arm.*;
import sun.jvm.hotspot.code.*;
import sun.jvm.hotspot.interpreter.*;
import sun.jvm.hotspot.runtime.*;
import sun.jvm.hotspot.runtime.arm.*;

/** <P> Should be able to be used on all arm platforms we support
    (Linux/arm) to implement JavaThread's "currentFrameGuess()"
    functionality. Input is an ARMThreadContext; output is SP, FP,
    and PC for an ARMFrame. Instantiation of the ARMFrame is
    left to the caller, since we may need to subclass ARMFrame to
...
public class ARMCurrentFrameGuess {
  ...
  private static final boolean DEBUG = System.getProperty("sun.jvm.hotspot.runtime.arm.ARMFrame.DEBUG")
                                       != null;

  public ARMCurrentFrameGuess(ARMThreadContext context,
                              JavaThread thread) {
  ...
  public boolean run(long regionInBytesToSearch) {
    Address sp  = context.getRegisterAsAddress(ARMThreadContext.SP);
    Address pc  = context.getRegisterAsAddress(ARMThreadContext.PC);
    Address fp  = context.getRegisterAsAddress(ARMThreadContext.R11);
    if (sp == null) {
      ...
      if (vm.isClientCompiler()) {
        ...
        for (long offset = 0;
          ...
          try {
            ...
            Frame frame = new ARMFrame(curSP, null, pc);
        ...
        // (unfortunately) duplicated from ARMFrame.senderForCompiledFrame.
        ...
        if (cc.contains(pc)) {
          ...
          // NOTE: This is the code duplicated from ARMFrame
  ...
  /** May be null if getting values from thread-local storage; take
      care to call the correct ARMFrame constructor to recover this if
      necessary */
